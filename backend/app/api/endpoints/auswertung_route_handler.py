...
@router.get("/evaluation_info_overview")
async def get_auswertung_info_overview(
    ...
    """
    Endpoint to retrieve traj_ids from info tables and corresponding traj_info details
    with pagination support.

    This endpoint does the following:
    1. Finds all info tables in the evaluation schema
    2. Retrieves unique traj_ids from these tables
    3. Fetches corresponding traj_info details with pagination
    """
    try:
        ...
        # 2. Create UNION query for all traj_ids
        union_queries = [
            f"SELECT DISTINCT traj_id FROM evaluation.{table['table_name']}"
            for table in tables
        ]
        bahn_ids_query = " UNION ".join(union_queries)

        # 3. Count total unique traj_ids for pagination
        count_query = f"""
        WITH unique_traj_ids AS ({bahn_ids_query})
...
@router.get("/evaluation_info/{bahn_id}")
async def get_auswertung_info_by_id(
    ...
    try:
        ...
        if not result:
            raise HTTPException(
                status_code=404,
                detail=f"No evaluation info found for traj_id {bahn_id}"
            )

        return result

    except Exception as e:
        logger.error(f"Error fetching evaluation info for {bahn_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Internal Server Error: {str(e)}")
...
async def get_auswertung_info(
    ...
    try:
        ...
        current_bahn_ids = [row['traj_id'] for row in bahn_info_rows]
        ...
        result["traj_info"] = [dict(row) for row in bahn_info_rows]
        ...
        for table in tables:
            ...
            WHERE traj_id IN ({bahn_ids_list})
            ORDER BY traj_id
            """
...
@router.get("/ed_evaluation/{bahn_id}")
@cache(expire=2400)
async def get_position_euclidean_by_id(bahn_id: str, conn=Depends(get_db)):
    try:
        ...
        if not rows:
            raise HTTPException(status_code=404, detail=f"No euclidean deviation data found for traj_id {bahn_id}")
...
async def get_position_dfd_by_id(bahn_id: str, conn=Depends(get_db)):
    try:
        ...
        if not rows:
            raise HTTPException(status_code=404, detail=f"No DFD deviation data found for traj_id {bahn_id}")
...
@router.get("/sidtw_evaluation/{bahn_id}")
@cache(expire=2400)
async def get_position_sidtw_by_id(bahn_id: str, conn=Depends(get_db)):
    try:
        ...
        if not rows:
            raise HTTPException(status_code=404, detail=f"No SIDTW deviation data found for traj_id {bahn_id}")
...
async def get_position_dtw_by_id(bahn_id: str, conn=Depends(get_db)):
    try:
        ...
        if not rows:
            raise HTTPException(status_code=404, detail=f"No DTW deviation data found for traj_id {bahn_id}")
...
async def search_auswertung_bahn_ids(
    ...
    try:
        # Der entscheidende Unterschied: Wir verbinden die bahn_info mit evaluation_ed_info
...
@router.get("/gd_evaluation/{bahn_id}")
async def get_qad_evaluation_by_id(bahn_id: str, conn=Depends(get_db)):
    try:
        ...
        if not rows:
            raise HTTPException(status_code=404, detail=f"No QAD orientation data found for traj_id {bahn_id}")
...
async def get_qdtw_evaluation_by_id(bahn_id: str, conn=Depends(get_db)):
    try:
        ...
        if not rows:
            raise HTTPException(status_code=404, detail=f"No QDTW deviation data found for traj_id {bahn_id}")
