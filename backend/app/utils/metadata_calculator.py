...
class MetadataCalculatorService:
    ...
    async def get_traj_ids_for_timerange(self, start_unix: int, end_unix: int) -> List[str]:
        """Ermittelt alle traj_ids im angegebenen Zeitraum basierend auf recording_date"""
        async with self.db_pool.acquire() as conn:
            ...
            query = """
                    ...
                    WHERE LEFT (recording_date \
                      ...
                      AND $2
                    GROUP BY traj_id
                    ORDER BY traj_id::text \
                    """
            rows = await conn.fetch(query, start_date, end_date)
            return [row['traj_id'] for row in rows]

    async def get_all_missing_traj_ids(self) -> tuple[List[str], List[str]]:
        """
        ...
        async with self.db_pool.acquire() as conn:
            ...
            missing_embeddings = [row['traj_id'] for row in embedding_rows]
    ...
    async def delete_existing_metadata(self, traj_ids: List[str]) -> int:
        """Löscht vorhandene Metadaten für gegebene traj_ids"""
        if not traj_ids:
            return 0
    ...
    async def delete_existing_embeddings(self, traj_ids: List[str]) -> int:
        """Löscht vorhandene Embeddings für gegebene traj_ids"""
        if not traj_ids:
            return 0
    ...
    async def _fetch_all_traj_data(self, conn: asyncpg.Connection, traj_id: str) -> Dict:
        ...
        joint_query = f"""
            ...
            GROUP BY seg_id
        """
        joints = await conn.fetch(joint_query, traj_id)
    ...
    def _calculate_all_metadata_in_memory(self, traj_id: str, traj_data: Dict) -> List[Dict]:
        ...
        """
        metadata_rows = []
        weight = traj_data['traj_info']['weight']

        # Berechne Gesamtdauer
        total_duration = 0.0
        if traj_data['segments'][0]['min_timestamp'] and traj_data['segments'][-1]['max_timestamp']:
            try:
                start_time = traj_data['segments'][0]['min_timestamp']
                end_time = traj_data['segments'][-1]['max_timestamp']
        ...
        for segment in traj_data['segments']:
            ...
            # Twist Stats
            twist_values = traj_data['twists'].get(seg_id, [])
            ...
            # Acceleration Stats
            accel_values = traj_data['accels'].get(seg_id, [])
        ...
        if metadata_rows:
            ...
            all_x = np.concatenate([np.array(seg['x_data']) for seg in traj_data['segments']])
            all_y = np.concatenate([np.array(seg['y_data']) for seg in traj_data['segments']])
            all_z = np.concatenate([np.array(seg['z_data']) for seg in traj_data['segments']])
            ...
            total_metadata = {
                'traj_id': traj_id,
                'seg_id': traj_id,  # Bahn-Level: seg_id = traj_id
    ...
    def _calculate_all_embeddings_in_memory(self, traj_id: str, traj_data: Dict) -> List[Dict]:
        """
        ✅ NEU: Berechnet Embeddings für alle Segmente UND die Gesamtbahn

        Returns:
            List[Dict] mit seg_id, traj_id, joint_embedding, position_embedding, orientation_embedding
        """
        embedding_rows = []

        # 1. Segmente (seg_id != traj_id)
        for segment in traj_data['segments']:
            seg_id = segment['seg_id']

            # Joint Embedding
            joint_emb = None
            joint_data_raw = traj_data['joints'].get(seg_id)
            ...
            ori_emb = None
            ori_data_raw = traj_data['orientations'].get(seg_id)
            ...
            segment_metadata = next(
                (m for m in traj_data.get('metadata_rows', []) if m['seg_id'] == seg_id),
                None
            )
            if segment_metadata:
        ...
        for segment in traj_data['segments']:
            ...
            # Joint
            joint_raw = traj_data['joints'].get(seg_id)
            ...
            # Orientation
            ori_raw = traj_data['orientations'].get(seg_id)
        ...
        traj_meta_emb = None
        traj_metadata = next(
            (m for m in traj_data.get('metadata_rows', []) if m['seg_id'] == traj_id),
            None
        )

        if traj_metadata:
            traj_meta_emb = self.embedding_calculator.compute_metadata_embedding(traj_metadata)

        # Berechne Bahn-Level Embeddings
        traj_joint_emb = self.embedding_calculator.compute_joint_embedding(all_joint_data) if all_joint_data else None
        traj_pos_emb = self.embedding_calculator.compute_position_embedding(all_pos_data) if all_pos_data else None
        traj_ori_emb = self.embedding_calculator.compute_orientation_embedding(all_ori_data) if all_ori_data else None
        traj_vel_emb = self.embedding_calculator.compute_velocity_embeddings(all_vel_data) if all_vel_data else None

        if traj_joint_emb is not None or traj_pos_emb is not None or traj_ori_emb is not None:
            embedding_rows.append({
                'seg_id': traj_id,
                'traj_id': traj_id,
                'joint_embedding': traj_joint_emb,
                'position_embedding': traj_pos_emb,
                'orientation_embedding': traj_ori_emb,
                'velocity_embedding': traj_vel_emb,
                'metadata_embedding': traj_meta_emb
            })
    ...
    async def process_single_traj(
        ...
        try:
            traj_data = await self._fetch_all_traj_data(conn, traj_id)
            ...
            if compute_metadata:
                ...
                metadata_rows = await self._load_existing_metadata(conn, traj_id)
                result['segments_processed'] = len(metadata_rows) - 1
            ...
            logger.error(f"Error working on traj_id {traj_id}: {e}")
            result['error'] = str(e)
    ...
    async def _load_existing_metadata(self, conn: asyncpg.Connection, traj_id: str) -> List[Dict]:
        """
    ...
    async def batch_write_metadata(
        ...
        logger.info(f"✓ Wrote {len(records)} metadata rows to traj_metadata")
    ...
    async def batch_write_embeddings_test(
        ...
        await conn.execute("""
                        INSERT INTO motion.traj_embeddings
                        (seg_id, traj_id, joint_embedding, position_embedding, orientation_embedding,
                        velocity_embedding, metadata_embedding)
                        SELECT seg_id,
                            traj_id,
        ...
        logger.info(f"✓ Wrote {len(records)} embedding rows to traj_embeddings")
    ...
    async def check_existing_embeddings(self, traj_ids: List[str]) -> List[str]:
        """
        ✅ NEU: Prüft welche traj_ids bereits Embeddings haben
        """
        if not traj_ids:
            return []
