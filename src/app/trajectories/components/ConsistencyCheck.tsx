...
import type { BahnTwistIst, BahnTwistSoll, BahnPoseIst, BahnPositionSoll, BahnEvents, BahnOrientationSoll, BahnAccelIst, BahnJointStates } from '@/types/main';
...
export const ConsistencyCheck: React.FC<ConsistencyCheckProps> = ({
  ...
  useEffect(() => {
    const checkConsistency = () => {
      try {
        ...
        const lastEventTime = Number(currentBahnEvents[currentBahnEvents.length - 1].timestamp);

        setTimeRange({ start: firstEventTime, end: lastEventTime });

        const filterDataByTimeRange = <T extends { timestamp: string }>(data: T[]): T[] => {
        ...
        Object.entries(dataArrays).forEach(([key, data]) => {
          if (data.length > 1) {
            const intervals = data.slice(1).map((d, i) => (Number(d.timestamp) - Number(data[i].timestamp)) / 1e9);
    ...
    checkConsistency();
  }, [currentBahnTwistIst, currentBahnTwistSoll, currentBahnPoseIst, idealTrajectory, currentBahnEvents, currentBahnOrientationSoll, currentBahnAccelIst, currentBahnJointStates]);

  if (error) {
    return (
      <div className="p-4 border rounded-md mb-4 bg-red-100">
        <h2 className="text-lg font-semibold mb-2">Fehler bei der Konsistenzprüfung</h2>
  ...
  return (
    <div className="p-4 border rounded-md mb-4">
      ...
      {timeRange && (
        <p className="mb-2">
          Analyse durchgeführt für Daten zwischen {new Date(timeRange.start / 1e6).toISOString()} und {new Date(timeRange.end / 1e6).toISOString()}
        </p>
      )}
      <table className="w-full border-collapse border border-gray-300">
        <thead>
          <tr className="bg-gray-100">
            ...
            <th className="border border-gray-300 p-2">Durchschn. Intervall (s)</th>
            <th className="border border-gray-300 p-2">Standardabw. Intervall (s)</th>
            <th className="border border-gray-300 p-2">Max. Lücke (s)</th>
            <th className="border border-gray-300 p-2">Variationskoeffizient (%)</th>
          </tr>
        </thead>
        <tbody>
          {Object.entries(metrics).map(([key, metric]) => (
            <tr key={key}>
              ...
              <td className="border border-gray-300 p-2">{metric.dataPoints}</td>
              <td className="border border-gray-300 p-2">{metric.avgInterval.toFixed(4)}</td>
              <td className="border border-gray-300 p-2">{metric.stdDevInterval.toFixed(4)}</td>
              <td className="border border-gray-300 p-2">{metric.maxGap.toFixed(4)}</td>
              <td className="border border-gray-300 p-2">{metric.coefficientOfVariation.toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
      </table>
