  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (useBatchUpload && files.length > 0) {
      try {
        ...
        const response = await fetch('/api/upload/process-csv-batch', {
          method: 'POST',
          body: formData,
        });
          ...
          <label className="mb-2 block text-base font-bold text-primary">
            ...
            <div className="mt-2">
              ...
              <label className="inline-flex items-center font-light">
                ...
                />
                <span className="ml-2">Split by reference position</span>
              </label>
            </div>
          </label>
        </div>
