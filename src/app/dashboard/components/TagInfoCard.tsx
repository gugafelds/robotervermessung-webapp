...
export function TagInfoCard({ selectedTags }: Props) {
  ...
  useEffect(() => {
    ...
    getTagInfo(selectedTags).then((r) =>
      setInfos((r as { tags: TagInfo[] }).tags ?? []),
    );
  }, [selectedTags]);
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
      {infos.map((info) => (
        <div
          key={info.tag}
          className="rounded-2xl border border-gray-500 bg-white p-4"
        >
          ...
          <div className="space-y-0">
            ...
            {row(
              'Velocity',
              info.vel_min != null && info.vel_max != null
                ? `${info.vel_min}–${info.vel_max} mm/s`
                : null,
            )}
            {row('Stop point', info.stop_point)}
            {row(
              'Reorientation XY',
              info.reorientation_xy ? `${info.reorientation_xy}°` : null,
            )}
            {row(
              'Reorientation Z',
              info.reorientation_z ? `${info.reorientation_z}°` : null,
            )}
            {row(
              'Min distance',
              info.min_distance ? `${info.min_distance} mm` : null,
            )}
            {row(
              'Workspace X',
              info.ws_x_min != null && info.ws_x_max != null
                ? `${info.ws_x_min}–${info.ws_x_max} mm`
                : null,
            )}
            {row(
              'Workspace Y',
              info.ws_y_min != null && info.ws_y_max != null
                ? `${info.ws_y_min}–${info.ws_y_max} mm`
                : null,
            )}
            {row(
              'Workspace Z',
              info.ws_z_min != null && info.ws_z_max != null
                ? `${info.ws_z_min}–${info.ws_z_max} mm`
                : null,
            )}
