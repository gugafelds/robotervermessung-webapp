...
function aggregate(rows: TimelinePoint[], tags: string[]): AggPoint[] {
  const src =
    tags.length > 0
      ? rows.filter((r) => r.tag !== null && tags.includes(r.tag))
      : rows;
  const byDate = new Map<
    string,
    { wsum: number; count: number; min: number; max: number }
  >();
  for (const r of src) {
...
export function AccuracyTimeline({ allTimeline, selectedTags, metric }: Props) {
  const [metricTimeline, setMetricTimeline] =
    useState<TimelinePoint[]>(allTimeline);
  const [isLoading, setIsLoading] = useState(false);

  // Re-fetch only when metric changes
  useEffect(() => {
    if (metric === 'sidtw') {
      setMetricTimeline(allTimeline);
      return;
    }
    setIsLoading(true);
    getMetricTimeline(metric)
      .then((r) =>
        setMetricTimeline((r as { timeline: TimelinePoint[] }).timeline ?? []),
      )
  ...
  const data = useMemo(
    () => aggregate(metricTimeline, selectedTags),
    [metricTimeline, selectedTags],
  );
  ...
  return (
    <div className="flex flex-col justify-center rounded-2xl border border-gray-500 bg-white p-6">
      ...
      <div className="mb-4 text-sm text-gray-600">
        <p>
          Development of measurement accuracy ({label}) over time. An increase
          could indicate wear and tear or calibration problems.
        </p>
      </div>
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div className="rounded-lg bg-green-50 p-4">
              ...
              <p className="text-2xl font-bold text-green-700">
                {Math.min(...data.map((d) => d.avg_val)).toFixed(3)} {unit}
              </p>
            </div>
            <div className="rounded-lg bg-red-50 p-4">
              <p className="text-sm text-gray-600">Worst Average</p>
              <p className="text-2xl font-bold text-red-700">
                {Math.max(...data.map((d) => d.avg_val)).toFixed(3)} {unit}
              </p>
            </div>
          </div>
        </>
